...
const images: GalleryItem[] = Array.from({ length: 12 }).map((_, i) => {
    const index = (i % 6) + 1;
    const available = i % 3 !== 0; // mark every 3rd as sold
    return {
        thumb: `/images/drawings/thumbs/work-${index}thumb.svg`,
        full: `/images/drawings/full/work-${index}.svg`,
        alt: `Drawing ${i + 1}`,
        title: `Drawing ${i + 1}`,
        status: available ? 'available' : 'sold',
        price: available ? `$${200 + i * 20}` : undefined,
    };
});
